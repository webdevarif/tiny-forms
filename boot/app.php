        ...
        if ($isNotCompatible) {
            add_action('admin_init', function () {
                $message = '<div style="padding: 15px 10px;" ><b>' . __('Heads UP: ',
                        'fluentform') . '</b>' . __('Tiny Forms Pro Plugin needs to be updated to the latest version.',
                        'fluentform') . '<a href="' . admin_url('plugins.php?s=fluentformpro&plugin_status=all&force-check=1') . '">' . __(' Please update Tiny Forms Pro to latest version.',
                        'fluentform') . '</a></div>';
                $actions = [
